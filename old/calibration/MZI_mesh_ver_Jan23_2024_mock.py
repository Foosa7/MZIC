...
from unittest.mock import MagicMock
# Mock classes for Qontrol and Thorlabs devices
class MockQontrol:
...
class MockThorlabsPM100:
    def __init__(self, inst=None):
        self.power = 0.123  # Simulate power reading
...
class MyInfoFrame(customtkinter.CTkFrame):
    def __init__(self, master, title, params):
        ...
        IMAGE_PATH = os.path.join(script_dir, 'logo_mini.png')
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Connecting to Mock devices
        print('')
        print("Connecting to QControl Device...")
        q = MockQontrol()  # Use the mock class instead of the real one
        print(f"Mock Qontroller '{q.device_id}' initialized with firmware {q.firmware} and {q.n_chs} channels.")
        params = {'Device id:': q.device_id, 'Available channels:': q.n_chs}
               # Connecting to Thorlabs P100D Powermeter Device
        print('')
        print("Connecting to Thorlabs P100D Powermeter Device...")
        rm = MagicMock()  # Mock ResourceManager
        rm.list_resources.return_value = ("USB0::0x1313::0x8078::PM001464::INSTR",)
        print("Available resources are ", rm.list_resources())
        
        inst = MagicMock()
        power_meter = MockThorlabsPM100(inst=inst)
