...
class MyTabView(customtkinter.CTkTabview):
    def __init__(self, master):
        ...
        self.functions_frame = Functions_frame(master = self.tab("Functions"), tabview=self)
        self.functions_frame.grid(row=0, column=0, padx=10, pady=(0,30), sticky="w")
...
class MyChannel_selection_frame(customtkinter.CTkFrame):
    def __init__(self, master, width, height, config, heaterid, **kwargs):
        ...
        self.IOconfigselect = customtkinter.CTkComboBox(self, width=75, values=self.app.IOconfig_options, command=lambda choice: self.IOconfig_change(choice))
        self.IOconfigselect.grid(column=3, row=0, padx=10, pady=10, sticky="sw")
        self.IOconfigselect.set("Cross")
        
        self.set_button = customtkinter.CTkButton(self, text="Freeze config", width=60, command=lambda: self.set_config(self.channelselect.get(), self.IOconfigselect.get()))
        self.set_button.grid(column=1, row=0, padx=10, pady=10, sticky="sw")

        self.rel_button = customtkinter.CTkButton(self, text="Release config", width=60, command=lambda: self.release_config(self.channelselect.get(), self.IOconfigselect.get()))
        self.rel_button.grid(column=2, row=0, padx=10, pady=10, sticky="sw")

        
        self.barstate_button = customtkinter.CTkButton(self, text = "Set bar state", width=60)
        self.barstate_button.grid(column=4, row=0, padx=(10,5), pady=10, sticky="nsew")
        
        self.crossstate_button = customtkinter.CTkButton(self, text = "Set cross state", width=60)
        self.crossstate_button.grid(column=5, row=0, padx=5, pady=10, sticky="nsew")
        
        self.halfstate_button = customtkinter.CTkButton(self, text = "Set 50:50 state", width=60)
        self.halfstate_button.grid(column=6, row=0, padx=(5,20), pady=10, sticky="nsew")
        
    def set_config(self, channel, ioconfig):
        
        self.IOconfigselect.configure(state="disabled")
        
        if channel == "Select channel":
            return
        else:
            self.app.set_ioconfig[self.channels.index(channel)] = self.IOconfigselect.get()
            self.app.set_fit[self.channels.index(channel)] = self.app.fit_func[1]

    
    def release_config(self, channel, ioconfig):
        
        self.IOconfigselect.configure(state="normal")
        
        if channel == "Select channel":
            return
        else:
            self.app.set_ioconfig[self.channels.index(channel)] = 'Null'
            self.app.set_fit[self.channels.index(channel)] = 'Null'
    ...
    def channel_change(self, choice):
        ...
        if self.app.set_ioconfig[self.app.channel] == 'Null':
            self.IOconfigselect.configure(state="normal")
            
        else:
            self.IOconfigselect.set(self.app.set_ioconfig[self.app.channel])
            self.IOconfigselect.configure(state="disabled")
            
        # if self.app.set_fit[self.app.channel] != 'Null':
        #     self.fitselect.configure(state="normal")
            
        # else:
        #     self.fitselect.set(self.app.set_fit[self.app.channel])
        #     self.fitselect.configure(state="disabled")
        
        #self.app.fit = self.fitselect.get()
        self.app.fit = self.app.fit_func[1]
    ...
    def IOconfig_change(self, choice):
        ...
        for i in range(self.app.config):
            if self.app.rmin_list[self.app.channel] != 'Null':
                self.app.scrollable_frame.acqresist_minpack[self.app.channel].configure(text=str(np.round(float(self.app.rmin_list[self.app.channel])/1000, 2)))
            if self.app.rmax_list[self.app.channel] != 'Null':
                self.app.scrollable_frame.acqresist_maxpack[self.app.channel].configure(text=str(np.round(float(self.app.rmax_list[self.app.channel])/1000, 2)))
            if self.app.alpha_list[self.app.channel] != 'Null':
                self.app.scrollable_frame.alphapack[self.app.channel].configure(text=str(np.round(float(self.app.alpha_list[self.app.channel])/1000, 2)))
            if self.app.linear_resistance_list[self.app.channel] != 'Null':
                self.app.scrollable_frame.acqresist_linpack[self.app.channel].configure(text=str(np.round(float(self.app.linear_resistance_list[self.app.channel])/1000, 2)))


class Functions_frame(customtkinter.CTkFrame):
    def __init__(self, master, tabview):
        super().__init__(master)
        
        self.tab = tabview
        self.app = self.tab.app
        
        self.title1 = customtkinter.CTkLabel(self, text="Nested sweep", font=customtkinter.CTkFont(family="Arial", size=14, weight="bold", underline=True), corner_radius=6)
        self.title1.grid(row=0, column=0, columnspan=3, padx=10, pady=(10,0), sticky="new")
        
        self.nochannel = customtkinter.CTkLabel(self, text="Select No. of channels:", corner_radius=6)
        self.nochannel.grid(row=1, column=0, columnspan=2, padx=10, pady=(10,0), sticky="nsew")
        
        self.number_options = ["1", "2", "3"]
        
        
        # self.entry = customtkinter.CTkComboBox(self,width=80, values=self.number_options)
        # self.entry.grid(row=1, column=2, padx=10, pady=(10,0), sticky="nsew")
        # self.entry.set(self.number_options[0])
        
        self.entry = customtkinter.CTkComboBox(self, width=80, values=self.number_options, command=self.update_column_visibility)
        self.entry.grid(row=1, column=2, padx=10, pady=(10,0), sticky="nsew")
        self.entry.set(self.number_options[0])  # Default value
        self.channel_num = self.entry
        
        self.choice1 = customtkinter.CTkLabel(self, text="Channel 1", corner_radius=6)
        self.choice1.grid(row=2, column=0, padx=10, pady=(10,0), sticky="nsew")
        
        self.choice2 = customtkinter.CTkLabel(self, text="Channel 2", corner_radius=6)
        self.choice2.grid(row=2, column=1, padx=10, pady=(10,0), sticky="nsew")
        
        self.choice3 = customtkinter.CTkLabel(self, text="Channel 3", corner_radius=6)
        self.choice3.grid(row=2, column=2, padx=10, pady=(10,0), sticky="nsew")
        
        
        self.channels=[]
        for i in range(int(self.app.config)):
            self.channels.append("Ch_"+str(i)+" ("+self.app.currentheaterid[i]+")")
        ...
        self.channel_selections = ['Null' for i in range(3)]
        self.start_phases = ['Null' for i in range(3)]
        self.end_phases = ['Null' for i in range(3)]
        self.steps = ['Null' for i in range(3)]
        self.current_phases = []
        self.current_currents = []

        
        # Adding widgets for columns dynamically
        for col in range(3):
            channel_select = customtkinter.CTkComboBox(self, width=120, values=self.channels)
            channel_select.grid(column=col, row=3, padx=10, pady=(10,0), sticky="nsew")
            channel_select.set("Select channel")
            self.column_widgets[col + 1].append(channel_select)
            self.channel_selections[col] = channel_select
            
            # if channel_select.get() != "Select channel":
            #     #self.channel_select[col] = self.channels.index(channel_select.get())
                
            
            
            start_phase = customtkinter.CTkEntry(self, placeholder_text="Start phase (π)", width=80)
            start_phase.grid(column=col, row=4, padx=20, pady=(10,0), sticky="nsew")
            self.column_widgets[col + 1].append(start_phase)
            self.start_phases[col] = start_phase
            
            end_phase = customtkinter.CTkEntry(self, placeholder_text="End phase (π)", width=80)
            end_phase.grid(column=col, row=5, padx=20, pady=(10,0), sticky="nsew")
            self.column_widgets[col + 1].append(end_phase)
            self.end_phases[col] = end_phase
            
            steps = customtkinter.CTkEntry(self, placeholder_text="Steps", width=80)
            steps.grid(column=col, row=6, padx=20, pady=(10,0), sticky="nsew")
            self.column_widgets[col + 1].append(steps)
            self.steps[col] = steps
            
            current_phase = customtkinter.CTkLabel(self, text="Null", corner_radius=6)
            current_phase.grid(row=7, column=col, padx=20, pady=(2,0), sticky="nsew")
            self.column_widgets[col + 1].append(current_phase)
            self.current_phases.append(current_phase)
            
            current_current = customtkinter.CTkLabel(self, text="Null", corner_radius=6)
            current_current.grid(row=8, column=col, padx=20, pady=(2,0), sticky="nsew")
            self.column_widgets[col + 1].append(current_current)
            self.current_currents.append(current_current)
            
        
        # Freeze controls based on the default value
        self.update_column_visibility()
        
        self.pausetimelabel = customtkinter.CTkLabel(self, text="Pause time:", corner_radius=6)
        self.pausetimelabel.grid(row=2, column=3, padx=10, pady=(10,0), sticky="nsew")
        
        self.pausetime = customtkinter.CTkEntry(self, placeholder_text="Seconds", width=80)
        self.pausetime.grid(column=3, row=3, padx=20, pady=(2,0), sticky="nsew")
        
        self.timer_display = customtkinter.CTkLabel(self, text="timer", corner_radius=6)
        self.timer_display.grid(row=4, column=3, padx=10, pady=(10,0), sticky="nsew")
        
        
        self.sweepbutton = customtkinter.CTkButton(self, text="Sweep", width=80, command=lambda channel_num=self.channel_num, channel_selections=self.channel_selections, start_phases=self.start_phases, end_phases=self.end_phases, steps=self.steps, pausetime=self.pausetime: self.app.sweep(channel_num, channel_selections, start_phases, end_phases, steps, pausetime))
        self.sweepbutton.grid(column=3, row=5, padx=10, pady=(10,0), sticky="nsew")

        
        self.stopbutton = customtkinter.CTkButton(self, text="Stop", width=80, command=lambda channel_num=self.channel_num, channel_selections=self.channel_selections: self.app.stop_sweep(channel_num, channel_selections))
        self.stopbutton.grid(column=3, row=6, padx=10, pady=(10,0), sticky="nsew")
        self.stopbutton.configure(state="disabled")
        
        self.statuslabel = customtkinter.CTkLabel(self, text="Idle!", corner_radius=6)
        self.statuslabel.grid(row=7, column=3, padx=10, pady=(10,0), sticky="nsew")

        
    def update_column_visibility(self, event=None):
        """Enable or disable widgets in columns based on the selected number of channels."""
        selected_channels = int(self.entry.get())
        
        for col, widgets in self.column_widgets.items():
            if col > selected_channels:
                # Disable widgets in this column
                for widget in widgets:
                    widget.configure(state="disabled")
            else:
                # Enable widgets in this column
                for widget in widgets:
...
class MyCalcFrame_main(customtkinter.CTkFrame):
    ...
    def applyphase(self):
        ...
        for i in range(len(phi)):
            # Check if the character is allowed in the input
            if phi[i] in self.app.allowedinputvalues:
                ...
                if phi[i] == '.':
                    point += 1
                    if point > 1:
                        print('Input has more than one decimal point')
                        break
        
            # Check for negative signs: only allow at the start or after an operator/space
            elif phi[i] == '-' and (i == 0 or phi[i-1] in self.app.allowedinputvalues + ['.', '-', ' ']):
                # Allow negative signs in the correct position
                print(phi[i])
            else:
...
class MyHeaderFrame(customtkinter.CTkFrame):
    def __init__(self, master, val, app):
        ...
        if self.val == '6x6' or self.val == '8x8' or self.val == 'custom':
            ...
            self.fitselect_all.set(self.app.default_fit)
            self.fitselect_all.configure(state="disabled")
            ...
            self.IOconfigselect_all.set(self.app.default_IOconfig)
...
class MyScrollableFrame(customtkinter.CTkScrollableFrame):
    def __init__(self, master, title, params, heaterid, config, app):
        ...
        self.app.default_fit = self.app.fit_func[1]
        self.app.IOconfig_options = ["Cross", "Bar"]
        self.app.default_IOconfig = self.app.IOconfig_options[0]
        self.app.fit_func_allchannels = [self.app.default_fit for i in range(self.config)] # changed this to select default fitting
        self.app.IOconfig_allchannels = [self.app.default_IOconfig for i in range(self.config)] 
        self.app.set_ioconfig = ['Null' for i in range(self.config)] # changed this to select default fitting
        self.app.set_fit = ['Null' for i in range(self.config)] 
        ...
        for i in range(self.config):
            ...
            self.fitselect.set(self.app.default_fit)
            ...
            self.IOconfigselect.set(self.app.default_IOconfig)
...
class App(customtkinter.CTk):
    def __init__(self, params, heaterid6x6, heaterid8x8, options, globalcurrrentlimit):
        super().__init__()
        ...
        self.tab_view = MyTabView(self.lowerframe)
        self.tab_view.grid(row=0, column=0, padx=10, pady=(0,10), sticky="nsew")
    ...
    def applyphaseforsweep(self, channel, phase, channel_index):
        
        fit = self.set_fit[channel]
        IOconfig = self.set_ioconfig[channel]
        ...
        self.currentarray = self.calpararrays[self.fit_func.index(self.set_fit[channel])][self.IOconfig_options.index(self.set_ioconfig[channel])]
        #self.currentarray = self.calpararrays[self.fit_index][self.IOconfig_index]
        
        self.currentsettings = self.currentarray[channel]
        ...
        if self.currentsettings != "Null":
            ...
            if fit == self.fit_func[0]:
                self.R = self.linear_resistance_list[channel] #get the resistance for the right channel    
                self.P_pi = abs((np.pi - self.c) / self.b) #Heating power of phi phase shift
 
                self.mod_period = 2*(np.pi)/self.b
                self.pi_PS_current = 1000*np.sqrt(self.P_pi/(self.R*1000))
                    
            elif fit == self.fit_func[1]:
                    
                # Define symbols
                I = sp.symbols('I')
                self.R0 = self.resistance_parameter_list[channel][1]
                self.alpha = self.resistance_parameter_list[channel][0]/self.resistance_parameter_list[channel][1]
        ...
        if self.currentsettings == "Null":
            messagebox.showerror(message='Phase shifter characterization data missing for the channel Ch_'+str(channel)+' ('+self.currentheaterid[channel]+'). Characterize the phase shiter before attempting to sweep the phase or current')
            self.stop_sweep_flag = True
            
        elif phase<self.c/np.pi:
            
            messagebox.showerror(message='Entered phase range for the channel Ch_'+str(channel)+' ('+self.currentheaterid[channel]+') inclue values less than  the offset phase of this phase shifter. Enter a greater value')
            self.stop_sweep_flag = True
            ...
            if illegalchar == True or point >1: #check for invalid characters in input
                messagebox.showerror(message='INVALID INPUT! \n Input value needs to be an integer or a float with a decimal point, not a comma. Please check the phase entries of channel Ch_'+str(channel)+' ('+self.currentheaterid[channel]+')')
                self.stop_sweep_flag = True
                
            else:
                phase = float(phase) #make the entry value a float (Unit of pi)
                self.phiphase2list[channel] = channel #save the entry value for the phase
                
                if fit == self.fit_func[0]:

                    P = abs((phase*np.pi - self.c) / self.b) #Heating power of phi phase shift
                    self.currenttoapply = float(round(1000 * np.sqrt(P/(self.R*1000)), 2)) #Derive the current in A, converted to mA to apply for this phase.
                    ...
                    P = abs((phase*np.pi - self.c) / self.b)
                    # Define equation
                    P=P/1000  #Unit in W
                ...
                self.derivedcurrentlist[channel] = self.current2apply #save the value of the current for this channel

        
                if self.derivedcurrentlist[channel] > q.imax[channel] or self.derivedcurrentlist[channel] < 0: 
                    messagebox.showwarning(message='WARNING! \n\n The current for phase '+str(phase)+'π and above for the channel Ch_'+str(channel)+' ('+self.currentheaterid[channel]+') is exceeding the current limit or is negative. Please apply smaller phase values')
                    self.stop_sweep_flag = True
                else: 
                    q.i[channel] = self.derivedcurrentlist[channel] #set the current to the derived current for the phase on the channel
                    #print(q.i[channel], channel)
                    self.setcurrentlist[channel] = self.derivedcurrentlist[channel] #save the actual value of the current that is set on the channel
                    self.tab_view.functions_frame.current_currents[channel_index].configure(text = str(round(self.setcurrentlist[channel], 3))+" mA")
                    self.tab_view.functions_frame.current_phases[channel_index].configure(text = str(round(phase, 3))+"π")



    def stop_sweep(self, channel_num, channel_select):
        
        self.stop_sweep_flag = True
        channel_num= int(channel_num.get())
        channel_select = [self.tab_view.functions_frame.channels.index(channel_select[i].get()) for i in range (0,channel_num)]
        
        """
        Stop the running sweep and reset all channels' phases to zero.
        """
        
        for i, channel in enumerate(channel_select[:channel_num]):
            
            q.i[channel] = 0
            self.setcurrentlist[channel] = 0 #save the current value to Null in the list for the right channel
            self.tab_view.functions_frame.current_currents[i].configure(text = str(self.setcurrentlist[channel])+" mA")
            self.tab_view.functions_frame.current_phases[i].configure(text = str(round(self.c/np.pi, 3))+"π")
            #messagebox.showinfo(message='Release of current on channel ' + str(channel) + ' and set to 0 mA')
            self.tab_view.functions_frame.statuslabel.configure(text="Idle!")
            self.tab_view.functions_frame.sweepbutton.configure(state="normal")
            self.tab_view.functions_frame.stopbutton.configure(state="disabled")
    

    def sweep(self, channel_num, channel_select, start_phases, end_phases, steps, pause_time):
        """Starts the sweep in a separate thread to prevent GUI freeze."""
        self.stop_sweep_flag = False  # Add a flag to stop the sweep when needed
    
        # Run the sweep in a separate thread
        thread = threading.Thread(target=self.sweep_process, args=(channel_num, channel_select, start_phases, end_phases, steps, pause_time))
        thread.daemon = True  # Daemonize the thread to stop when the app closes
        thread.start()
    
    def sweep_process(self, channel_num, channel_select, start_phases, end_phases, steps, pause_time):
        """Performs the sweep process in a loop."""
        if self.stop_sweep_flag:  # Immediately exit if stop button was pressed
            return 
        
        channel_num= int(channel_num.get())
        
        for i in range(channel_num):
            if self.stop_sweep_flag == True:  # Check if the user pressed "Stop"
                break
    
            if any(value == "Select channel" for value in [channel_select[i].get() for i in range (0,channel_num)]):
                ...
                channel_select = [self.tab_view.functions_frame.channels.index(channel_select[i].get()) for i in range (0,channel_num)]
                start_phases = [start_phases[i].get() for i in range (0,channel_num)]
                end_phases = [end_phases[i].get() for i in range (0,channel_num)]
                steps = [steps[i].get() for i in range (0,channel_num)]
                
                if any(value == '' for value in start_phases + end_phases + steps):
                    messagebox.showerror(
                        "Error",
                        "Please enter the start phase, end phase and step size for all selected channels to proceed with sweep."
                    )
                    return
                    ...
                    if pause_time.get() == '':
                        messagebox.showerror(
                            "Error",
                            "Please Enter the pause time in seconds."
                        )
                        return
                    else:
                
                        start_phases = [float(start_phases[i]) for i in range (0,channel_num)]
                        end_phases = [float(end_phases[i]) for i in range (0,channel_num)]
                        steps = [int(steps[i]) for i in range (0,channel_num)]
                        pause_time=float(pause_time.get())
                        
                        # Check for invalid step counts
                        if any(not isinstance(int(step), int) or int(step) <= 0 for step in steps):
                            ...
                            messagebox.showerror(
                                "Error",
                                "Step count must be a positive integer for all channels."
                            )
                            return
                        else:
                            
                            
                            if any(self.set_ioconfig[channel] == 'Null' for channel in channel_select):
                                
                                
                                messagebox.showerror(
                                    "Error",
                                    'Freeze the IOconfigs to be considered for all the channels'
                                )
                                return
                            
                            else:
                                if any(self.set_fit[channel] == 'Null' for channel in channel_select):
                                    
                                    messagebox.showerror(message='Set the fitting function to be considered for all the channels')
                                    return
                            
                                else:
                            
                                    self.tab_view.functions_frame.statuslabel.configure(text="Running...")  # Indicate that the process has started
                                    self.tab_view.functions_frame.sweepbutton.configure(state="disabled")
                                    self.tab_view.functions_frame.stopbutton.configure(state="normal")
                                    
                                    # Recursive nested loop implementation
                                    def nested_loop(level, current_phases):
                                        if level == channel_num:
                                            # Base case: All levels completed, apply phases
                                            for i, phase in enumerate(current_phases):
                                                if self.stop_sweep_flag == True:  # Check if the user pressed "Stop"
                                                    break
                                                self.applyphaseforsweep(channel_select[i], phase, i)
                            
                                                # Pause with timer countdown
                                            if pause_time.is_integer():
                                                for t in range(int(pause_time), 0, -1):
                                                    if self.stop_sweep_flag == True:  # Check if the user pressed "Stop"
                                                        break
                                                    self.tab_view.functions_frame.timer_display.configure(text=str(t))
                                                    self.update_idletasks()  # Force the GUI to update
                                                    self.after(1000)  # Schedule the next iteration after 1 second
                                                    
                                            else:
                                                
                                                self.update_idletasks()  # Force the GUI to update
                                                self.after(int(pause_time*1000))  # Schedule the next iteration after 1 second
                                                    
                                            print("\n")
                                            return
                            
                                        # Generate loop for the current level
                                        start = start_phases[level]
                                        end = end_phases[level]
                                        step_count = steps[level]
                            
                                        if step_count == 1:
                                            phases = [start]
                                        else:
                                            step_size = (end - start) / (step_count - 1)
                                            phases = [start + i * step_size for i in range(step_count)]
                            
                                        for phase in phases:
                                            if self.stop_sweep_flag == True:  # Check if the user pressed "Stop"
                                                break
                                            nested_loop(level + 1, current_phases + [phase])
                            
                                    # Start the recursive nested loop
                                    nested_loop(0, [])
                                    ...
                                    for i, channel in enumerate(channel_select[:channel_num]):
                                        ...
                                        offset_phase=self.currentsettings["phase"]
                                        self.tab_view.functions_frame.current_phases[i].configure(text = str(round(offset_phase/np.pi, 3))+"π")
                                        #messagebox.showinfo(message='Release of current on channel ' + str(channel) + ' and set to 0 mA')
                                    ...
                                    self.tab_view.functions_frame.sweepbutton.configure(state="normal")
                                    self.tab_view.functions_frame.stopbutton.configure(state="disabled")
                                    #self.stop_sweep_flag = False  # Reset the stop flag
    ...
    def calibration_func(self, channel, rampstart, rampend, steps, stabtime, acqlinresist, acqvariying_resist_param):
        ...
        if (self.numresist == 'Null').any(): # added this to check any resistance value or else it is Null
...
if __name__ == "__main__":
    ...
    try:
        ...
        inst = rm.open_resource('USB0::0x1313::0x8078::P0039134::INSTR') #ASRL4::INSTR 
        ...
        app = App(params, heaterid6x6, heaterid8x8, options, globalcurrrentlimit)
        app.mainloop()
