...
def import_default_pkl(obj):
    """
    Reads default pkl from json config file and imports the data into the given object 'obj'.
    ...
    # Import the pickle file using the config
    imported = import_pickle(config)
    if imported is None:
        raise FileNotFoundError("Failed to load the default pickle file. Please check the configuration.")
    ...
    import_dir = os.path.dirname(config.get("default_config", ""))
